...
ISACTest::Setup()
{
    ...
    _fileNameSWB = webrtc::test::ProjectRootPath() + "test/data/audio_coding/testfile32kHz.pcm";
    ...
    _inFileA.Open(_fileNameSWB.c_str(), 32000, "rb");
    ...
    ACMTestISACConfig& swbISACConfig)
{
    ...
    // Files in Side A 
    _inFileA.Open(_fileNameSWB.c_str(), 32000, "rb", true);
    if(_testMode == 0)
    {
        sprintf(fileNameOut,
    ...
    // Files in Side B
    _inFileB.Open(_fileNameSWB.c_str(), 32000, "rb", true);
    if(_testMode == 0)
    {
        sprintf(fileNameOut,
                "%s/out_iSACTest_%s_%02d.pcm",
    ...
    int maxSampRateChange)
{
    ...
    // Files in Side B
    _inFileB.Open(_fileNameSWB.c_str(), 32000, "rb");
    if(_testMode == 0)
    {
        sprintf(fileNameOut,
                "%s/out_iSACTest_%s_%02d.pcm",
